...
*Setup instructions for a Gitian build of Earthcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Earthcoin
...
- [Building Earthcoin Core](#building-earthcoin-core)
...
Clone the git repositories for earthcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/earthcoin/earthcoin.git
git clone https://github.com/earthcoin/gitian.sigs.git
```
...
in the earthcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Earthcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the earthcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/earthcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/earthcoin/earthcoin
...
For example:
```bash
URL=https://github.com/laanwj/earthcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit earthcoin=${COMMIT} --url earthcoin=${URL} ../earthcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit earthcoin=${COMMIT} --url earthcoin=${URL} ../earthcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit earthcoin=${COMMIT} --url earthcoin=${URL} ../earthcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the earthcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../earthcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/earthcoin/earthcoin-detached-sigs.git

BTCPATH=/some/root/path/earthcoin
SIGPATH=/some/root/path/earthcoin-detached-sigs

./bin/gbuild --url earthcoin=${BTCPATH},signature=${SIGPATH} ../earthcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/earthcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/earthcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/earthcoin-osx-build.assert
```
...
[earthcoin/gitian.sigs](https://github.com/earthcoin/gitian.sigs/) repository, or if that's not possible create a pull
