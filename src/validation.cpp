                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetEarthcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Earthcoin: BIP30 has been active since inception
    ...
    // Earthcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetEarthcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if( !block.isLegacy() && (block.GetBaseVersion() < 3 && nHeight >= consensusParams.BIP66Height) )
    ...
    // Earthcoin: Introduce supermajority rules for v4 blocks
